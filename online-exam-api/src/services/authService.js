...
const bulkRegister = async (usersData) => {
  const created = [];
  const defaultPassword = 'Student@12345'; // default for bulk imported users if not provided
  for (const data of usersData) {
    if (!data.email || !data.name) continue;
    const normalizedEmail = String(data.email).toLowerCase().trim();
    let existing = await userRepository.findByEmail(normalizedEmail);
    if (!existing) {
