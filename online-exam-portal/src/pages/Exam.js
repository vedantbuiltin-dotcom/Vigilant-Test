...
import { FALLBACK_EXAM_DATA, FALLBACK_ANSWERS } from "../data/fallbackExamData";

const ExamPage = ({ logOut }) => {
  const [cq, setCQ] = useState(1);
  const [data, setData] = useState();
  const [answers, setAnswers] = useState(null);
  const [useseFallback, setUsesFallback] = useState(false);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        console.log("API Error - Loading fallback data:", err.message);
        // Use fallback data if API fails
        setData(FALLBACK_EXAM_DATA);
    ...
    <div>
      {useseFallback && (
        <Alert severity="info" sx={{ mb: 2 }}>
          📝 Demo Mode: Using fallback exam data. API server not available.
        </Alert>
      )}
